...
def run_evidence_inference(model, data_loader):

    # get the evidence function
    if model.cls_head.loss_cls.evidence == 'relu':
        from mmaction.models.losses.edl_loss import relu_evidence as get_evidence
    elif model.cls_head.loss_cls.evidence == 'exp':
        from mmaction.models.losses.edl_loss import exp_evidence as get_evidence
    elif model.cls_head.loss_cls.evidence == 'softplus':
...
def run_inference():
    ...
    if not args.uncertainty == 'EDL':
        ...
        all_uncertainties = run_evidence_inference(model, data_loader)
    return all_uncertainties
